package com.jty.msmservice.service.impl;
...
public class MsmServiceImpl implements MsmService {
    ...
    public boolean sendMail(String to, String code) {
        if(StringUtils.isEmpty(to)) return false;
        try {
            SimpleMailMessage message = new SimpleMailMessage();
            System.out.println(from);
            message.setFrom(from);
            message.setTo(to);
            message.setSubject("[CSA学苑]验证码");
            message.setText("亲爱的用户，您的CSA学苑注册验证码为："+code+" 。如非本人操作请无视。");
